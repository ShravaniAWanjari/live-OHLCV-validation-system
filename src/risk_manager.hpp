...
enum class PipelineState { ACTIVE, HALTED };

class RiskManager {
public:
  RiskManager(int max_non_critical_errors = 3)
      : state_(PipelineState::ACTIVE), non_critical_error_count_(0),
  ...
  void handle_validation_result(uint8_t validation_flags,
    ...
    if (critical) {
      trigger_halt("Critical validation failure(Flags: " +
                   std::to_string(validation_flags) + ")" +
                   std::string(context));
    } else {
...
private:
  void trigger_halt(const std::string &reason) {
    ...
    std::cerr
        << "\n################################################################"
        << std::endl;
    std::cerr << "!!! RISK MANAGER CRITICAL ALERT : SYSTEM HALTING !!!"
              << std::endl;
    std::cerr << "Reason: " << reason << std::endl;
    std::cerr << "ACTION: PULLING ALL ACTIVE ORDERS FROM EXCHANGE IMMEDIATELY!"
              << std::endl;
    std::cerr << "STATUS: DOWNSTREAM TRADING STRATEGY DISCONNECTED."
              << std::endl;
    std::cerr
        << "################################################################\n"
        << std::endl;
